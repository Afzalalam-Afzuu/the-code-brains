...
export default function LearningSidebar({ course, currentLessonId }: LearningSidebarProps) {
  ...
  return (
    <>
      {/* Mobile Top Chapter Toggle Bar */}
      <div className="lg:hidden sticky top-[98px] z-30 bg-slate-900 border-b border-slate-800 px-4 py-2.5 flex items-center justify-between text-white shadow-md">
        <button
          onClick={() => setIsMobileOpen(true)}
          className="flex items-center gap-2 text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg transition shadow-sm"
        >
          <Menu size={16} />
      ...
      {/* Mobile Modal Drawer & Backdrop Overlay */}
      {isMobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-80 max-w-[85vw] bg-slate-950 border-r border-slate-800 text-slate-200 flex flex-col h-full shadow-2xl z-50">
            {/* Mobile Header with Close Button */}
            <div className="p-4 border-b border-slate-800 bg-slate-900 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-xl">{course.icon}</span>
                <h2 className="font-extrabold text-xs text-white uppercase tracking-wider">{course.title}</h2>
              </div>
              <button
                onClick={() => setIsMobileOpen(false)}
            ...
            {/* Progress Bar inside Mobile Drawer */}
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              <div className="px-2 py-1 text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1">
      ...
      <aside className="hidden lg:flex sticky top-0 h-[calc(100vh-2rem)] w-72 bg-slate-950 border-r border-slate-800 text-slate-200 flex-col shrink-0">
